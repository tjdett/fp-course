...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  f <$> st = StateT (\s -> let sf (a, s') = (f a, s') in sf <$> runStateT st s)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k b
  stf <*> sta = StateT $ \s0 ->
    runStateT stf s0 >>= (\(f, s1) ->
      runStateT sta s1 >>= (\(a, s2) ->
        pure (f a, s2)
      )
    )
...
distinct' ::
  ...
  List a
  -> List a
distinct' = (`eval'` S.empty)
  . filtering (\v ->
      getT >>= \seen ->
        not (v `S.member` seen) <$ putT (S.insert v seen))
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  OptionalT kof <*> OptionalT koa = OptionalT $
    kof >>= onFull (\f -> (f <$>) <$> koa)
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  f =<< OptionalT koa = OptionalT $ onFull (runOptionalT . f) =<< koa
...
distinctG l = runOptionalT $ evalT (filtering p l) S.empty
 where
   liftStateOptionalT :: (s -> Logger Chars (Optional (a, s)))
                      -> StateT s (OptionalT (Logger Chars)) a
   liftStateOptionalT f = StateT (OptionalT . f)
   abortAt100 :: (Integral a, Show a)
             => a
             -> StateT (S.Set a) (OptionalT (Logger Chars)) ()
   abortAt100 v = liftStateOptionalT $ \s ->
     if v > 100
       then log1 ("aborting > 100: " ++ show' v) (Empty)
       else pure . pure $ ((), s)
   logIsEven :: (Integral a, Show a)
             => a
             -> StateT (S.Set a) (OptionalT (Logger Chars)) ()
   logIsEven v = liftStateOptionalT $ \s ->
     if even v
       then log1 ("even number: " ++ show' v) (pure $ ((), s))
       else pure . pure $ ((), s)
   p :: (Integral a, Show a)
     => a
     -> StateT (S.Set a) (OptionalT (Logger Chars)) Bool
   p v = getT >>= \seen ->
     putT (S.insert v seen)
       *> abortAt100 v
       *> logIsEven v
       *> pure (v `S.notMember` seen)
