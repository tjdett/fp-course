...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = (\c -> if p c then valueParser c else unexpectedCharParser c)
   =<< character 
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany 0 _ = pure Nil
thisMany n p = p .:. thisMany (n-1) p
...
surnameParser ::
  Parser Chars
surnameParser = (\s -> (s ++) <$> list lower) =<< upper .:. thisMany 5 lower
...
phoneParser ::
  Parser Chars
phoneParser = (digit .:. phoneBodyParser) >>= (\v -> v <$ is '#')
