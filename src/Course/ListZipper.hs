...
toList ::
  ListZipper a
  -> List a
toList (ListZipper Nil h t) = h :. t
toList (ListZipper (ph :. pt) v t) = toList $ ListZipper pt ph (v :. t)
...
fromList (h :. t) = MLZ (Full (ListZipper Nil h t))
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l v r2) = case break p l of
  (_, Nil) -> MLZ Empty
  (r1, (v' :. l')) -> MLZ . Full $ ListZipper l' v' (foldLeft (flip (:.)) (v :. r2) r1)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l1 v r) = case break p r of
  (_, Nil) -> MLZ Empty
  (l2, (v' :. r')) -> MLZ . Full $ ListZipper (foldLeft (flip (:.)) (v :. l1) l2) v' r' 
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper (ph :. pt) v t) = ListZipper pt ph (v :. t)
moveLeftLoop (ListZipper Nil v r) = ListZipper l' v' Nil
 where (v' :. l') = foldLeft (flip (:.)) (v :. Nil) r
...
moveRightLoop (ListZipper l v Nil) = ListZipper Nil v' r'
 where (v' :. r') = foldLeft (flip (:.)) (v :. Nil) l
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN 0 = isZ
moveLeftN n | n > 0     = asMaybeZipper (moveLeftN (n - 1)) . moveLeft
...
moveLeftN' 0 = Right . id
moveLeftN' n | n > 0     = \lz -> (?? Left (n - 1)) . (Right <$>) . toOptional . moveLeft P.=<< moveLeftN' (n - 1) lz
             | otherwise = \lz -> (?? Left (abs (n + 1))) . (Right <$>) . toOptional . moveRight P.=<< moveLeftN' (n + 1) lz
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n lz = moveRightN (n - length (lefts lz)) lz
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lz = if hasRight lz then end (moveRightLoop lz) else lz
...
start ::
  ListZipper a
  -> ListZipper a
start lz = if hasLeft lz then start (moveLeftLoop lz) else lz
...
deletePullLeft (ListZipper (v :. l) _ r) = isZ $ ListZipper l v r
...
deletePullRight (ListZipper l _ (v :. r)) = isZ $ ListZipper l v r
...
-- /Tip:/ Use `zipWith`
  ListZipper lf vf rf <*> ListZipper la va ra = ListZipper (zipWith ($) lf la)
                                                           (vf va)
                                                           (zipWith ($) rf ra)
...
instance Extend ListZipper where
  f <<= lz = ListZipper (unfoldrL moveLeft) (f lz) (unfoldrL moveRight)
   where
    unfoldrL moveF = unfoldr (((\v -> (f v, v)) <$>) . toOptional . moveF) lz
