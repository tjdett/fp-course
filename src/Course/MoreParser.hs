...
hex ::
  Parser Char
hex = replicateA 4 (satisfy isHexDigit) >>= \s -> case readHex s of
  Empty -> P $ const (UnexpectedString s)
  Full n -> pure (chr n)
...
eof ::
  Parser ()
eof = P $ \s -> if s == "" then Result s () else ExpectedEof s
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll preds = satisfy (\c -> all ($ c) preds)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny preds = satisfy (\c -> any ($ c) preds)
